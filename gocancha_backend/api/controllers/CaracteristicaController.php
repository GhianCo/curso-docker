...
class CaracteristicaController { 
    ...
    public function getById($id) {
    	...
    	$mensajes = array();
    	$tipo = SUCCESS;
    	$datos = Caracteristica::getById($id);
    	if(!$datos){
    ...
    public function listarPorPaginacion($busqueda,$pagina,$registros) {
      ...
      if($busqueda != REST_TODOS){
      	$sqlWhere[] = array('field'=>'caracteristica_descripcion','value'=>$busqueda,'operator'=>'=');
      }
    	$array_salida = Caracteristica::getByFields($sqlWhere,array(),($pagina-1)*$registros,$registros);
    	$totalCount=$array_salida['totalCount'];
    	$array_salida=$array_salida['caracteristica_array'];
    	$datos=$array_salida;
    	$data['mensajes'] = $mensajes;
    	$data['tipo'] = $tipo;
    	$data['data'] = $datos;
    ...
    public function getAllActivos() {
    	...
    	$datos = Caracteristica::getAllActivos();
