...
class BloqueoController { 
    ...
    public function listarPorPaginacion($busqueda,$pagina,$registros) {
      ...
      if($busqueda != REST_TODOS){
      	$sqlWhere[] = array('field'=>'bloqueo_descripcion','value'=>$busqueda,'operator'=>'=');
      }
    	$array_salida = Bloqueo::getByFields($sqlWhere,array(),($pagina-1)*$registros,$registros);
    	$totalCount=$array_salida['totalCount'];
    	$array_salida=$array_salida['bloqueo_array'];
    	$datos=$array_salida;
    	$data['mensajes'] = $mensajes;
    	$data['tipo'] = $tipo;
    	$data['data'] = $datos;
    ...
    public function getAllActivos() {
    	...
    	$mensajes = array();
    	$tipo = SUCCESS;
    	$array_salida = Bloqueo::getByFields(array(
    	array('field'=>'bloqueo_estado','value'=>'1','operator'=>'=')
    	));
    	$datos=$array_salida['bloqueo_array'];
    	$data['mensajes'] = $mensajes;
    	$data['tipo'] = $tipo;
    	$data['data'] = $datos;
    	return $data;
